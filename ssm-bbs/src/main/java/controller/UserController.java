...
public class UserController {
    ...
    public ModelAndView login(User user, HttpServletRequest request) throws Exception {
        ...
        if (lUser == null) {
            ...
            List<Tab> list1= tabService.findAll();
            mv.addObject("tabs",list1);
    ...
    public void addFavorite(@RequestParam("tip_id") String tip_id,HttpSession session) throws Exception {
        User user = (User) session.getAttribute("user");
        int user_id = user.getUser_id();
        userService.addFavorite(user_id, tip_id);
    }

    @ResponseBody
    @RequestMapping("isFavorite.do")
    public String isFavorite(@RequestParam("tip_id") String tip_id,HttpSession session) throws Exception {
        User user = (User) session.getAttribute("user");
        int user_id = user.getUser_id();
        String flag = String.valueOf(userService.isFavorite(user_id, tip_id));
        return flag;
    }
