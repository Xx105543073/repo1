...
public interface TipDao {
    ...
    @Select("select * from tip order by tip_publishTime desc")
    @Results({
    ...
    @Insert("insert into tip (user_id,tab_id,tip_title,tip_content,tip_publishTime) values(#{user_id},#{tab_id},#{tip.tip_title},#{tip.tip_content},#{tip.tip_publishTime})")
    ...
    @Select("select * from tip where user_id=#{user_id} order by tip_publishTime desc")
    @Results({
            @Result(id = true, property = "tip_id", column = "tip_id"),
            @Result(property = "user", column = "user_id", javaType = User.class, one = @One(select = "dao.UserDao.findById")),
            @Result(property = "tab", column = "tab_id", javaType = Tab.class, one = @One(select = "dao.TabDao.findById")),
    ...
    @Select("SELECT * FROM tip WHERE tip_title LIKE CONCAT('%',#{word},'%') OR tip_content LIKE CONCAT('%',#{word},'%') order by tip_publishTime desc")
    @Results({
            @Result(id = true, property = "tip_id", column = "tip_id"),
            @Result(property = "user", column = "user_id", javaType = User.class, one = @One(select = "dao.UserDao.findById")),
            @Result(property = "tab", column = "tab_id", javaType = Tab.class, one = @One(select = "dao.TabDao.findById")),
    ...
    @Select("select * from tip where tip_id in(select tip_id from favorite where user_id =#{user_id}) order by tip_publishTime desc")
    @Results({
            @Result(id = true, property = "tip_id", column = "tip_id"),
            @Result(property = "user", column = "user_id", javaType = User.class, one = @One(select = "dao.UserDao.findById")),
            @Result(property = "tab", column = "tab_id", javaType = Tab.class, one = @One(select = "dao.TabDao.findById")),
    ...
    @Delete("delete from reply where tip_id=#{id}")
